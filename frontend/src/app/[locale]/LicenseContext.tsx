...
export const LicenseProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    // Sekme kapatıldığında veya component söküldüğünde interval'ı temizle
  ...
  const offlineBanner = daysOffline >= 3 ? (
    <div
      style={{
        ...
        background: daysOffline >= 6
          ? 'linear-gradient(90deg, #7f1d1d, #991b1b)'
          : 'linear-gradient(90deg, #78350f, #92400e)',
        color: '#fef3c7',
        borderBottom: daysOffline >= 6 ? '2px solid #ef4444' : '2px solid #f59e0b',
        boxShadow: '0 2px 12px rgba(0,0,0,0.4)',
      }}
    >
      <span style={{ fontSize: '16px' }}>{daysOffline >= 6 ? '🔴' : '⚠️'}</span>
      <span>
        {daysOffline >= 6
          ? `⚠️ KRİTİK: Sunucuya ${daysOffline} gündür bağlanamıyoruz. İnternet bağlantısı sağlanmazsa sistem ${remainingDays} gün içinde kilitlenecek!`
          : `Sunucuya ${daysOffline} gündür ulaşılamıyor. Lisans kilitlenmesine ${remainingDays} gün kaldı. Lütfen internet bağlantısınızı kontrol edin.`
        }
      </span>
    </div>
  ) : null;
  ...
  return (
    <LicenseContext.Provider value={{ isValid, modules, daysOffline, loading, refreshLicense: fetchStatus, hasModule }}>
      {offlineBanner}
      <div style={daysOffline >= 3 ? { paddingTop: '42px' } : undefined}>
        {children}
      </div>
