...
export function PageClient() {
  ...
  const handleEditSale = async (sale: Sale) => {
        const result = await showSwal({
            title: 'Satış Geri Yüklensin mi?',
            text: "Bu satış masaya geri yüklenecektir. İlgili masa tekrar açılacak ve adisyon masaya eklenecektir.",
            icon: 'question',
        ...
        if (result.isConfirmed) {
            try {
                const token = localStorage.getItem('token') || (user as any)?.token;
                const res = await fetch(`${API_URL}/sales/${sale.id}/restore`, {
                    method: 'POST',
                    headers: { Authorization: `Bearer ${token}` }
                });
                
                if (res.ok) {
                    toastSwal({ icon: 'success', title: 'Satış masaya geri yüklendi.' });
                    router.push(`/${locale}/pos?tableId=${sale.tableId}`);
                } else {
                    const data = await res.json();
                    showSwal({ icon: 'error', title: 'Hata', text: data.message || 'Satış geri yüklenemedi.' });
                }
            } catch (error) {
                console.error('Error restoring sale:', error);
                showSwal({ icon: 'error', title: 'Hata', text: 'Bağlantı hatası.' });
            }
        }
    };
