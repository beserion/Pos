...
const FullCalendar = dynamic(() => import('@fullcalendar/react'), { 
    ssr: false,
    loading: () => <div className="h-full flex items-center justify-center bg-white/50 dark:bg-slate-800/50 backdrop-blur-md rounded-3xl">Takvim Yükleniyor...</div>
});
...
export function PageClient() {
    ...
    useEffect(() => {
        ...
        if (user) {
            if (hasFeature('reservation_system')) {
                fetchData();
            } else {
                setLoading(false);
            }
