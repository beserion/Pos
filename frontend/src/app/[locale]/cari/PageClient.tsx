...
export function PageClient() {
    ...
    const txWithBalance = [...safeTx].reverse().map(tx => {
        runningBalance += tx.type === 'INCOME' ? Number(tx.amount) : -Number(tx.amount);
        return { ...tx, runningBalance };
    }).reverse();

    const totalIncome = safeTx.filter(t => t.type === 'INCOME').reduce((s, t) => s + Number(t.amount), 0);
    const totalExpense = safeTx.filter(t => t.type === 'EXPENSE').reduce((s, t) => s + Number(t.amount), 0);
