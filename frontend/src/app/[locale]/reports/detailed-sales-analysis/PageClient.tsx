          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-8">
              {[
                ...
                { label: 'İade (Genel)', value: `${fmt(data.genelToplamlar.toplamIade)} ₺`, color: 'bg-slate-50 dark:bg-slate-500/10 text-slate-600 dark:text-slate-400 border-slate-100 dark:border-slate-500/20' },
                { label: 'İptal Ürün', value: `${fmt(data.genelToplamlar.toplamIptal)} ₺`, color: 'bg-orange-50 dark:bg-orange-500/10 text-orange-700 dark:text-orange-300 border-orange-100 dark:border-orange-500/20' },
                { label: 'İade Ürün', value: `${fmt(data.genelToplamlar.toplamIadeUrun)} ₺`, color: 'bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-300 border-rose-100 dark:border-rose-500/20' },
              ].map((c) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              ...
              <Section title="Tahsilat Kırılımı (Ödeme Yöntemine Göre)">
                {data.tahsilatDetay?.map((t: any, i: number) => (
                  <Row key={i} label={getPaymentMethodLabel(t.odemeYontemi)} sub={`${t.adet} adet`} value={`${fmt(t.toplam)} ₺`} />
                ))}
              </Section>

              {/* İndirim Özeti */}
              <Section title="İşlem Özeti">
