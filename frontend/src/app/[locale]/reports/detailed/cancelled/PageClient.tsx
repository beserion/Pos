...
export function PageClient() {
  ...
  const fetchData = async (start = startDate, end = endDate) => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API_URL}/reports/cancelled?startDate=${start}&endDate=${end}`,
        {
  ...
  const handleExportExcel = () => {
    ...
    const headers = ['Masa', 'Adisyon No', 'Tarih/Saat', 'Yetkili', 'Ürün', 'Adet', 'Kategori', 'İptal Nedeni', 'İptal Tutarı'];
    ...
    exportToExcel(`Iptal_Raporu_${startDate}_${endDate}`, [
      { name: 'İptaller', headers, rows }
    ]);
  };
  ...
  const handlePrint80mm = () => {
    ...
    const summaryItems = [
      ...
      { label: 'TOPLAM TUTAR', value: `${fmt(totalCancelAmount)} TL`, bold: true },
      { label: 'TOPLAM ADISYON', value: String(totalCancelTransactions) }
    ];
  ...
  const filteredItems = data?.items?.filter((item: any) => {
    ...
    return (
      ...
      (item.cancelReason && item.cancelReason.toLowerCase().includes(term))
    );
  }) || [];

  // Dinamik KPI hesaplamaları
  const totalQuantity = filteredItems.reduce((acc: number, curr: any) => acc + Number(curr.quantity || 0), 0);
  const totalCancelAmount = filteredItems.reduce((acc: number, curr: any) => acc + Number(curr.total || 0), 0);
  const totalCancelTransactions = new Set(filteredItems.map((item: any) => item.saleId)).size;
    ...
    <div className="w-full min-h-screen font-sans relative transition-colors duration-300 bg-slate-955">
      ...
      */}
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden print:hidden">
        {/* Sol üst neon parıltısı - Amber/Orange Tonları */}
        <div className="absolute -top-[15%] -left-[10%] w-[600px] h-[600px] rounded-full bg-gradient-to-tr from-amber-600/15 to-orange-600/20 blur-[130px] animate-[pulse_8s_infinite_alternate]" />
        {/* Sağ alt neon parıltısı */}
        <div className="absolute -bottom-[15%] -right-[10%] w-[600px] h-[600px] rounded-full bg-gradient-to-br from-orange-500/10 to-amber-600/15 blur-[130px] animate-[pulse_10s_infinite_alternate]" />
      </div>
        ...
        <div className="relative z-10 w-full max-w-8xl mx-auto flex-1 flex flex-col">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="mb-0 text-3xl font-extralight text-amber-400 leading-none uppercase tracking-[0.25em]">
                  İptal Detay Raporu
                </h3>
                <h5 className="text-slate-400 mb-0 text-lg font-medium mt-1.5 leading-relaxed">
                  Adisyonlardan iptal edilen ürünlerin masa, yetkili personel ve kategori bazlı finansal dökümü.
                </h5>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <button
                ...
                className="px-6 py-3 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-400 font-black text-xs uppercase tracking-widest rounded-2xl shadow-sm hover:shadow-[0_4px_20px_-3px_rgba(245,158,11,0.4)] transition-all flex items-center gap-2 hover:scale-105 active:scale-95 cursor-pointer disabled:opacity-50 disabled:pointer-events-none"
              >
          ...
          <div className="bg-white/5 dark:bg-slate-900/30 backdrop-blur-2xl p-6 rounded-[32px] border border-white/10 dark:border-slate-800/60 shadow-[0_20px_40px_rgba(0,0,0,0.2)] mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button 
                onClick={() => fetchData()} 
                className="flex items-center gap-2 px-5 py-3 bg-amber-600 hover:bg-amber-700 text-white rounded-2xl font-bold text-xs uppercase tracking-wider shadow-md shadow-amber-500/20 transition-all hover:scale-102 active:scale-95"
              >
            ...
            <div className="relative flex-1 max-w-md w-full">
              ...
              <input
                ...
                className="w-full bg-slate-900/40 border border-slate-800 rounded-2xl pl-12 pr-4 py-3 text-slate-200 placeholder-slate-500 text-sm font-medium outline-none focus:ring-2 focus:ring-amber-500/50 transition-all"
              />
            </div>
          </div>
          ...
          {isLoading ? (
            <div className="flex-1 flex flex-col items-center justify-center py-32">
              <div className="relative w-16 h-16 rounded-full border-4 border-amber-500/30 border-t-amber-500 animate-spin" />
              <p className="text-sm text-slate-400 font-bold uppercase tracking-widest mt-6 animate-pulse">İptal Raporu Yükleniyor...</p>
            </div>
          ) : !data || filteredItems.length === 0 ? (
            <div className="bg-white/5 dark:bg-slate-900/30 backdrop-blur-2xl p-16 rounded-[40px] border border-white/10 dark:border-slate-800/60 shadow-[0_20px_50px_rgba(0,0,0,0.3)] text-center flex flex-col items-center justify-center min-h-[300px]">
              <Info size={40} className="text-amber-400/60 mb-4 animate-bounce" />
              <h4 className="text-xl font-black text-white leading-tight uppercase tracking-wider mb-2">Veri Bulunamadı</h4>
              <p className="text-sm text-slate-400 max-w-sm leading-relaxed">Seçilen tarihlerde ve arama filtrelerinde iptal işlemi bulunmamaktadır.</p>
            </div>
          ) : (
            <>
              {/* SUMMARY CARDS (KPIs) */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                {/* Kart 1 - Toplam İptal Adeti (Amber) */}
                <div className="bg-white/5 dark:bg-slate-900/30 backdrop-blur-xl p-6 rounded-[32px] border border-white/10 dark:border-slate-800/60 flex items-center justify-between transition-all hover:border-amber-300 dark:hover:border-amber-500/40 hover:shadow-[0_8px_30px_-5px_rgba(245,158,11,0.3)] hover:scale-[1.02] cursor-pointer">
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Toplam İptal Adeti</p>
                ...
                {/* Kart 2 - Toplam İptal Değeri */}
                <div className="bg-white/5 dark:bg-slate-900/30 backdrop-blur-xl p-6 rounded-[32px] border border-white/10 dark:border-slate-800/60 flex items-center justify-between transition-all hover:border-orange-300 dark:hover:border-orange-500/40 hover:shadow-[0_8px_30px_-5px_rgba(249,115,22,0.3)] hover:scale-[1.02] cursor-pointer">
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Toplam İptal Değeri</p>
                    <h3 className="text-3xl font-black text-slate-800 dark:text-white">{fmt(totalCancelAmount)} ₺</h3>
                  </div>
                ...
                {/* Kart 3 - Toplam İptal Adisyon Sayısı */}
                <div className="bg-white/5 dark:bg-slate-900/30 backdrop-blur-xl p-6 rounded-[32px] border border-white/10 dark:border-slate-800/60 flex items-center justify-between transition-all hover:border-yellow-300 dark:hover:border-yellow-500/40 hover:shadow-[0_8px_30px_-5px_rgba(234,179,8,0.3)] hover:scale-[1.02] cursor-pointer">
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Adisyon İşlem Sayısı</p>
                    <h3 className="text-3xl font-black text-slate-800 dark:text-white">{totalCancelTransactions} Adisyon</h3>
                  </div>
                  <div className="w-16 h-16 rounded-2xl bg-yellow-50 dark:bg-yellow-500/10 flex items-center justify-center text-yellow-600 dark:text-yellow-400">
                    <User size={28} />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="lg:col-span-3 bg-white/5 dark:bg-slate-900/30 backdrop-blur-2xl rounded-[32px] border border-white/10 dark:border-slate-800/60 p-6 md:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.2)]">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-lg font-black text-white uppercase tracking-wider">İptal Detay Listesi</h3>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-slate-800 dark:border-slate-800/50 text-[11px] font-black text-slate-400 uppercase tracking-widest">
                          ...
                          <th className="py-4 px-3">İptal Nedeni</th>
                          <th className="py-4 px-3 text-right">İptal Tutarı</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-800/40 dark:divide-slate-800/20">
                        {filteredItems.map((item: any) => (
                          <tr key={item.id} className="text-xs text-slate-300 hover:bg-white/5 dark:hover:bg-slate-900/20 transition-all">
                            ...
                            <td className="py-2 px-3 italic text-slate-400 max-w-[180px] truncate" title={item.cancelReason || '-'}>
                              {item.cancelReason || '-'}
                            </td>
                            <td className="py-2 px-3 text-right font-black text-amber-400">{fmt(item.total)} ₺</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="bg-white/5 dark:bg-slate-900/30 backdrop-blur-2xl rounded-[32px] border border-white/10 dark:border-slate-800/60 p-6 shadow-[0_20px_50px_rgba(0,0,0,0.2)]">
                    <h4 className="text-xs font-black text-white uppercase tracking-widest mb-4 flex items-center gap-2">
                      <Grid size={14} className="text-amber-400" /> Kategori & Cins Dağılımı
                    </h4>
                  ...
                  <div className="bg-white/5 dark:bg-slate-900/30 backdrop-blur-2xl rounded-[32px] border border-white/10 dark:border-slate-800/60 p-6 shadow-[0_20px_50px_rgba(0,0,0,0.2)]">
                    <h4 className="text-xs font-black text-white uppercase tracking-widest mb-4 flex items-center gap-2">
                      <User size={14} className="text-amber-400" /> Yetkili Aktivitesi
                    </h4>
                      ...
                      {data?.staffSummary?.map((staff: any, idx: number) => (
                        <div key={idx} className="flex items-center justify-between p-2.5 bg-slate-900/30 rounded-xl border border-slate-800/40">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-lg bg-amber-500/10 text-amber-400 flex items-center justify-center font-black text-xs">
                              {idx + 1}
                            </div>
                          ...
                          <div className="text-right shrink-0">
                            ...
                            <div className="text-[10px] font-bold text-amber-400">{fmt(staff.totalValue)} ₺</div>
                          </div>
                        </div>
                      ))}
                    </div>
      ...
      <div className="hidden print:block print-only-layout w-full bg-white text-black p-2 font-sans text-xs">
        ...
        <div className="border-b-2 border-slate-950 pb-1 mb-2 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h2 className="text-xs font-bold text-slate-800 tracking-wide mt-0.5 uppercase">İPTAL DETAY RAPORU</h2>
        ...
        <div className="grid grid-cols-3 gap-2 mb-2 border border-slate-300 bg-slate-50/50 p-2 rounded-lg avoid-break">
          <div className="text-center border-r border-slate-200">
            <span className="text-[8px] font-bold text-slate-500 uppercase tracking-wider">Toplam İptal Adeti</span>
            <div className="text-sm font-black text-slate-900 mt-0.5">{totalQuantity} Adet</div>
          </div>
          <div className="text-center border-r border-slate-200">
            <span className="text-[8px] font-bold text-slate-500 uppercase tracking-wider">Toplam İptal Değeri</span>
            <div className="text-sm font-black text-slate-900 mt-0.5">{fmt(totalCancelAmount)} ₺</div>
          </div>
          <div className="text-center">
            <span className="text-[8px] font-bold text-slate-500 uppercase tracking-wider">İptal Yapılan Farklı Adisyon</span>
            <div className="text-sm font-black text-slate-900 mt-0.5">{totalCancelTransactions} Adet</div>
          </div>
        </div>
        ...
        <div className="mb-2">
          <h3 className="text-[9.5px] font-bold text-slate-900 uppercase tracking-wider mb-1 border-l-4 border-slate-800 pl-1.5">
            1. İptal Detay Listesi ({filteredItems.length} Kayıt)
          </h3>
          <table className="print-table-corp">
            <thead>
              <tr>
                ...
                <th style={{ width: '24%' }}>İptal Edilen Ürünler / Miktar</th>
                <th style={{ width: '18%' }}>İptal Nedeni</th>
                <th style={{ width: '16%', textAlign: 'right' }}>İptal Tutarı</th>
              </tr>
            </thead>
            <tbody>
              {filteredItems.map((item: any) => (
                <tr key={item.id}>
                  ...
                  <td className="italic text-slate-600">{item.cancelReason || '-'}</td>
