...
export function PageClient() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const rawStocks = Array.isArray(stocksRes.data) ? stocksRes.data : (stocksRes.data?.data || []);
            setStockCards(rawStocks.filter((c: any) => c.isActive !== false));
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isDuplicate) {
                ...
                let msg = '';
                if (dup?.sku === formData.sku) msg = `"${formData.sku}" kodlu ürün zaten mevcut.`;
                else if (dup?.name === formData.name) msg = `"${formData.name}" isimli ürün zaten mevcut.`;
                else if (dup?.barcode === formData.barcode) msg = `"${formData.barcode}" barkodlu ürün zaten mevcut.`;

                showSwal({ title: tc('error'), text: msg, icon: 'warning' });
                return;
            }

            // Prepare final payload with recipe if applicable
            const finalPayload = { ...payload };
            
            // Clean and add recipeHeader if currentRecipe exists and has lines
            if (hasRecipeLicense && currentRecipe && (currentRecipe.lines || []).length > 0) {
                // Veriyi temizle: Reçete satırlarındaki stockCard nesnesini ve ana nesnedeki product nesnesini çıkar
                // Ayrıca id: 0 ise onu da çıkar (yeni reçete durumu)
                const { product, id, ...cleanRecipe } = currentRecipe as any;
                const sanitizedLines = (currentRecipe.lines || []).map((line: any) => {
                    const { stockCard, id: lineId, ...cleanLine } = line;
                    return cleanLine;
                });
                
                (finalPayload as any).recipeHeader = { 
                    ...cleanRecipe, 
    ...
    const handleSaveRecipe = async () => {
        ...
        if (currentRecipe.productId === 0) {
            showSwal({ 
                title: 'Bilgi', 
                text: 'Yeni ürünler için reçete, ürünle birlikte kaydedilir. Lütfen modalın altındaki "Kaydet" butonuna basın.', 
                icon: 'info' 
            });
            return;
        }
                                                            ...
                                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                                ...
                                                                {formData.inventoryLinkType === 'direct_stock' && (
                                                                    <>
                                                                        ...
                                                                        <div>
                                                                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">DÜŞÜLECEK MİKTAR ({getUnitName(formData.directStockUnit || '') || 'Birim'})</label>
                                                                            <div className="relative">
                                                                                <input
                                                                                    type="number"
                                                                                    step="0.0001"
                                                                    ...
                                                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-end">
                                                                        ...
                                                                        {v.inventoryLinkType === 'direct_stock' && (
                                                                            <>
                                                                                    ...
                                                                                    <SearchableSelect
                                                                                        ...
                                                                                        options={[
                                                                                            ...
                                                                                            ...(Array.isArray(stockCards) ? stockCards : []).map((sc: any) => ({
                                                                                                ...
                                                                                                label: `${sc.name || ''} (${getUnitName(sc.baseUnit || sc.unit || '')})`
                                                                                            }))
                                                                                        ]}
                                                                                        icon="fat fa-box"
                                                                                    />
                                                                                ...
                                                                                <div>
                                                                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">DÜŞÜLECEK MİKTAR ({getUnitName(v.linkedStockItemId ? stockCards.find((sc: any) => sc.id === v.linkedStockItemId)?.baseUnit || stockCards.find((sc: any) => sc.id === v.linkedStockItemId)?.unit || '' : '') || 'Birimi'})</label>
