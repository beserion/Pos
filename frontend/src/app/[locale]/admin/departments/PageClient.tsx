...
export function PageClient() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        if (isDuplicate) {
            showSwal({ title: 'Hata', text: `"${formData.name}" isimli kategori zaten mevcut.`, icon: 'warning' });
            return;
        }
    ...
    const handleParentGroupSave = async (e: React.FormEvent) => {
        ...
        // Duplicate check (frontend)
        const isDuplicate = parentGroups.some(pg => 
            pg.id !== parentGroupFormData.id && 
            pg.name.toLowerCase() === parentGroupFormData.name.toLowerCase()
        );

        if (isDuplicate) {
            showSwal({ title: 'Hata', text: `"${parentGroupFormData.name}" isimli üst grup zaten mevcut.`, icon: 'warning' });
            return;
        }
        ...
        try {
            ...
            showSwal({ title: 'Hata', text: err?.response?.data?.message || 'Üst grup kayıt hatası', icon: 'error' });
        }
    };
