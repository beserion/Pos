...
export function PageClient() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [empRes, delRes] = await Promise.all([
                ...
                axios.get(API_URL + '/deliveries', { headers: { Authorization: `Bearer ${user.token}` } })
            ]);
    ...
    const handleAssignCourier = async (e: React.FormEvent) => {
        ...
        try {
            await axios.put(`${API_URL}/deliveries/${selectedDeliveryId}`,
    ...
    const handleCancelDelivery = async (deliveryId: string) => {
        ...
        try {
            await axios.put(`${API_URL}/deliveries/${deliveryId}`,
                { status: 'CANCELLED' },
                { headers: { Authorization: `Bearer ${user.token}` } }
            );
