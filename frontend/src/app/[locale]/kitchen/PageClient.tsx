...
export function PageClient() {
    ...
    const fetchKitchenOrders = async () => {
        try {
            ...
            const res = await axios.get(`${API_URL}/sales/kitchen`, {
                            ...
                            >
                                <div className="p-3 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700 flex justify-between items-start">
                                    ...
                                    <div className={`text-right ${isSevere ? 'text-rose-500 dark:text-rose-400 font-bold animate-pulse' : isWarning ? 'text-amber-500 dark:text-amber-400 font-bold' : 'text-slate-500 dark:text-slate-300'}`}>
                                        ...
                                        <span className="text-lg">{formatElapsedTime(ticket.createdAt)}</span>
                                    </div>
                                </div>
                                ...
                                <div className="p-3 flex-1 bg-white dark:bg-slate-800/40">
                                    <ul className="space-y-2">
                                        {ticket.items.map((item, idx) => (
                                            <li key={idx} className="flex gap-2 text-base group-hover:bg-slate-50 dark:group-hover:bg-slate-700/50 p-2 rounded-lg -mx-1 transition-colors border border-transparent dark:hover:border-slate-600/50">
                                                <span className="font-black text-indigo-500 dark:text-indigo-400 text-lg">{item.quantity}x</span>
                                                <div>
                                                    <span className="font-bold text-slate-800 dark:text-white block text-lg">{item.product?.name}</span>
                                    ...
                                    {ticket.status === 'NEW' ? (
                                        ...
                                        <>
                                            <button
                                                ...
                                                className="py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-lg transition shadow-sm flex items-center justify-center gap-2"
                                            >
