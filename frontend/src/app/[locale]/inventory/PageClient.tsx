...
import { UBL_UNITS, getUnitName } from '@/app/[locale]/utils/units';
...
export function PageClient() {
    ...
    const handlePrintMovements = () => {
        ...
        const rows = movements.map((m, idx) => {
            ...
            <tr style="background:${rowBg};border-bottom:0.5px solid #e2e8f0;">
                ...
                <td style="padding:3px 6px;text-align:right;font-weight:900;font-size:8.5px;color:${color}">${isPositive ? '+' : ''}${Number(m.quantity).toLocaleString('tr-TR', { maximumFractionDigits: 2 })} <span style="font-size:6px;color:#94a3b8;margin-left:2px;font-weight:700;">${getUnitName(selectedStockCardForMovements.baseUnit)}</span></td>
                <td style="padding:3px 6px;text-align:right;font-weight:900;font-size:8.5px;color:#1e293b;">${Number(m.stockAfter).toLocaleString('tr-TR', { maximumFractionDigits: 2 })} <span style="font-size:6px;color:#94a3b8;margin-left:2px;font-weight:700;">${getUnitName(selectedStockCardForMovements.baseUnit)}</span></td>
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Duplicate checks (frontend)
            const isDuplicate = stockCards.some(c => 
                c.id !== formData.id && (
                    (formData.code && c.code === formData.code) ||
                    (formData.name && c.name === formData.name) ||
                    (formData.barcode && c.barcode === formData.barcode)
                )
            );

            if (isDuplicate) {
                const dup = stockCards.find(c => 
                    c.id !== formData.id && (
                        (formData.code && c.code === formData.code) ||
                        (formData.name && c.name === formData.name) ||
                        (formData.barcode && c.barcode === formData.barcode)
                ...
                let msg = '';
                if (dup?.code === formData.code) msg = `"${formData.code}" kodu zaten kullanımda.`;
                else if (dup?.name === formData.name) msg = `"${formData.name}" ismi zaten mevcut.`;
                else if (dup?.barcode === formData.barcode) msg = `"${formData.barcode}" barkodu zaten kullanımda.`;
                
                showSwal({ title: tc('error'), text: msg, icon: 'warning' });
                return;
            }
    ...
    const handleGroupSave = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!user?.token || !groupFormData.name) return;

        const nameToSave = groupFormData.name;

        // Duplicate check (frontend)
        const isDuplicate = stockGroups.some(g => 
            g.id !== groupFormData.id && 
            g.name.toLowerCase() === nameToSave.toLowerCase()
        );

        if (isDuplicate) {
            showSwal({ title: 'Hata', text: `"${nameToSave}" isimli stok grubu zaten mevcut.`, icon: 'warning' });
            return;
        }
        ...
        try {
            ...
            showSwal({ title: 'Hata', text: error.response?.data?.message || 'Grup kaydedilemedi.', icon: 'error' });
        }
    };
    ...
    const generateStockCode = (categoryName?: string) => {
        const prefix = (categoryName && categoryName.length >= 2) 
            ? categoryName.substring(0, 3).toLocaleUpperCase('tr') 
            : 'STK';
    ...
    const openModal = (card?: StockCard) => {
        ...
        if (card) {
            ...
            if (!hasRecipeLicense) {
                ...
                updatedCard.baseUnit = 'NIU';
            }
            setFormData(updatedCard);
        } else {
            const defaultVat = availableTaxRates.length > 0 ? parseFloat(availableTaxRates[0]) : 20;
            const defaultWarehouseId = warehouses.length > 0 ? Math.min(...warehouses.map(w => w.id)) : null;
            setFormData({
                id: 0,
                name: '',
                                                            ...
                                                            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 min-w-[150px]">
                                                                <div className="text-right">
                                                                    <span className="text-[11px] font-black text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-500/10 px-2 py-1 rounded border border-teal-100 dark:border-teal-500/20 whitespace-nowrap">
                                                                        {card.conversionRate} {getUnitName(card.baseUnit)}
                                                                    </span>
                                                                </div>
                                                ...
                                                <td className="px-8 py-4">
                                                    <div className="flex justify-center">
                                                        <div className={`inline-flex items-baseline gap-1.5 ${isOutOfStock ? 'text-red-500' : isLowStock ? 'text-amber-500' : 'text-slate-700 dark:text-slate-300'}`}>
                                                            ...
                                                            <span className="text-[10px] font-bold uppercase opacity-60 tracking-widest">{getUnitName(card.baseUnit)}</span>
                                                ...
                                                <td className="px-8 py-4">
                                                    <div className="text-center">
                                                        <div className="font-bold text-slate-600 dark:text-slate-400">
                                                            ₺{Number(card.costPerBaseUnit).toFixed(4)} <span className="text-[10px] opacity-50 uppercase">/{getUnitName(card.baseUnit)}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                    ...
                                                    <SearchableSelect
                                                        ...
                                                        onChange={(val) => {
                                                            ...
                                                            if (group) {
                                                                ...
                                                                const newCode = generateStockCode();
                                                                setFormData(prev => ({
                                                                    ...prev,
                                                                    stockGroupId: null,
                                                                    category: '',
                                            ...
                                            <div>
                                                ...
                                                <div className="-m-2 w-full">
                                                    <SearchableSelect
                                                        ...
                                                        onChange={(val) => setFormData({ ...formData, purchaseUnit: val.toString() })}
                                                        options={[
                                                            { value: '', label: 'Seçiniz...' },
                                                            ...UBL_UNITS
                                                        ]}
                                                    ...
                                                    return (
                                                        <tr key={idx} className="hover:bg-indigo-500/5 transition-all">
                                                            ...
                                                            <td className="px-6 py-3 text-center">
                                                                ...
                                                                <span className="text-[10px] font-bold text-slate-400 ml-1 uppercase">{getUnitName(selectedStockCardForMovements?.baseUnit || '')}</span>
                                                            </td>
