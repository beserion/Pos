...
export function PageClient() {
    ...
    const loadDependencies = useCallback(async () => {
        ...
        try {
            const whRes = await axios.get(`${API}/warehouses`, config);
            const whData = Array.isArray(whRes.data) ? whRes.data : (whRes.data as any)?.data || [];
            setWarehouses(whData.filter((w: any) => w.isActive));
        } catch (err: any) {
            console.error("Warehouses load failed:", err.response?.data || err.message);
        }
    }, [currentUser?.token]);
    ...
    const openEditInvoice = async (inv: Invoice) => {
        try {
            ...
            setFormItems((data.items || []).map(item => ({
                ...
                discountRate1: Number(item.discountRate1) || 0,
                discountRate2: Number(item.discountRate2) || 0,
    ...
    // ── Add Item Row ─────────────────────────────────────────────────────────────
    const addItemRow = () => {
        const defaultWarehouseId = warehouses.length > 0 ? Math.min(...warehouses.map(w => w.id)) : undefined;
        setFormItems(prev => [...prev, {
            stockCardId: 0, stockCardName: '', quantity: 1, unit: 'NIU',
            unitPrice: 0, vatRate: 20, discountRate1: 0, discountRate2: 0, 
            description: '', warehouseId: defaultWarehouseId,
        }]);
    };
        ...
        setFormItems(prev => {
            ...
            if (field === 'stockCardId') {
                ...
                if (card) {
                    ...
                    updated[idx].unit = card.purchaseUnit || card.baseUnit || 'NIU';
    ...
    const calcLineTotalDiscounted = (item: InvoiceItem) => {
    ...
    const calcLineDiscount = (item: InvoiceItem) => calcLineTotal(item) - calcLineTotalDiscounted(item);
    
    const calcLineVat = (item: InvoiceItem) => calcLineTotalDiscounted(item) * (Number(item.vatRate) || 0) / 100;
    
    const calcLineTotalWithVat = (item: InvoiceItem) => calcLineTotalDiscounted(item) + calcLineVat(item);

    const totalDiscountAmount = formItems.reduce((s, i) => s + calcLineDiscount(i), 0);
    const totalVat = formItems.reduce((s, i) => s + calcLineVat(i), 0);
    const grandTotal = subtotal - totalDiscountAmount + totalVat;
        ...
        try {
            const payload = {
                ...
                items: formItems.map(i => ({
                    ...
                    discountRate1: Number(i.discountRate1 || 0),
                    discountRate2: Number(i.discountRate2 || 0),
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-slate-50 dark:bg-slate-900/80 sticky top-0 z-20 backdrop-blur-sm">
                                    ...
                                    <th className="text-left px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest w-40">Depo</th>
                                    <th className="text-center px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest w-28">Miktar</th>
                                    <th className="text-center px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest w-24">Birim</th>
                                    <th className="text-center px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest w-32">Birim Fiyat</th>
                                    <th className="text-center px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest w-20">İsk 1 %</th>
                                    <th className="text-center px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest w-20">İsk 2 %</th>
                                    <th className="text-center px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest w-24">KDV %</th>
                                    <th className="text-right px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest w-32">Tutar (Net)</th>
                                    <th className="text-right px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest w-32">KDV Dahil</th>
                                    ...
                                    <tr key={idx} className="border-t border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                        ...
                                        <td className="px-4 py-2">
                                            <div className="-m-2">
                                                <SearchableSelect 
                                                    ...
                                                    onChange={val => updateItem(idx, 'warehouseId', Number(val) || undefined)}
                                                    options={[
                                                        { value: 0, label: 'Depo seçin...' },
                                        ...
                                        <td className="px-4 py-2">
                                            <input type="number" value={item.unitPrice} min={0} step={0.01} onChange={e => updateItem(idx, 'unitPrice', Number(e.target.value))}
                                                className="w-full px-2 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-center text-slate-800 dark:text-white text-xs outline-none focus:ring-2 focus:ring-indigo-500/20" />
                                        </td>
                                        <td className="px-1 py-2">
                                            <input type="number" value={item.discountRate1 ?? 0} min={0} max={100} step={0.1} onChange={e => updateItem(idx, 'discountRate1', Number(e.target.value))}
                                                className="w-full px-1 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-center text-rose-500 font-bold text-xs outline-none focus:ring-2 focus:ring-rose-500/20" />
                                        </td>
                                        <td className="px-1 py-2">
                                            <input type="number" value={item.discountRate2 ?? 0} min={0} max={100} step={0.1} onChange={e => updateItem(idx, 'discountRate2', Number(e.target.value))}
                                                className="w-full px-1 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-center text-rose-500 font-bold text-xs outline-none focus:ring-2 focus:ring-rose-500/20" />
                                        </td>
                                        <td className="px-4 py-2">
                                            <div className="-m-2">
                                        ...
                                        <td className="px-4 py-2 text-right font-bold text-slate-700 dark:text-slate-200">₺{fmt(calcLineTotalDiscounted(item))}</td>
                    ...
                    <div className="flex justify-end mt-4">
                        <div className="w-full max-w-sm space-y-1.5">
                            ...
                            <div className="flex justify-between items-center text-xs text-slate-500">
                                ...
                                <span className="font-bold text-rose-500">-₺{fmt(totalDiscountAmount)}</span>
                            </div>
