    ...
    useEffect(() => {
        const fetchStatus = async () => {
            try {
                ...
                setProgramDate(res.data.activeBusinessDate);
            } catch (error) {
            ...
            <nav className="relative z-20 bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl shadow-sm border-b border-white/20 dark:border-slate-700/50 transition-colors">
                <div className="w-full px-[50px] h-24 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        {programDate && (
                            <div className="hidden lg:flex items-center gap-3 px-4 py-2 rounded-2xl bg-indigo-500/10 dark:bg-indigo-500/20 border border-indigo-500/20 backdrop-blur-sm shadow-sm ring-1 ring-indigo-500/10 transition-all hover:bg-indigo-500/20 dark:hover:bg-indigo-500/30">
                                <i className="fat fa-calendar-star text-indigo-500 dark:text-indigo-400 text-xl"></i>
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-bold text-indigo-500/60 dark:text-indigo-400/60 uppercase tracking-widest leading-none mb-1">Program Tarihi</span>
                                    <span className="text-sm font-black text-indigo-700 dark:text-indigo-300 leading-none">
                                        {new Date(programDate).toLocaleDateString('tr-TR', { day: '2-digit', month: 'long', year: 'numeric' })}
                                    </span>
                                </div>
                            </div>
                        )}
