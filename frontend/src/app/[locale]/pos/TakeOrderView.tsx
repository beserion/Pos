...
interface OrderItem { product: Product; quantity: number; note?: string; isWaiting?: boolean; subItems?: any[]; extraPrice?: number; uniqueId?: string; saleType?: 'STANDARD' | 'HALF' | 'DOUBLE'; saleTypeMultiplier?: number; variationId?: number; variationName?: string; transactionType?: 'SALE' | 'FREE' | 'COMPLIMENTARY' | 'PROMOTION' | 'STAFF' | 'TICKET'; transactionReason?: string; status?: string; refundReason?: string; cancelReason?: string; discountRate?: number; discountAmount?: number; }
interface ExistingOrder {
    id: number;
    totalAmount: number;
    items: { id: number; product: { id: number; name: string; price: number; isSet?: boolean }; quantity: number; unitPrice: number; isPaid: boolean; isWaiting: boolean; isMarshed: boolean; parentItemId?: number; addedByName?: string; addedAt?: string; transactionType?: string; transactionReason?: string; status?: string; refundReason?: string; cancelReason?: string; discountRate?: number; discountAmount?: number; }[];
}
...
export default function TakeOrderView({ onSwitchToPos, onCancelAdisyon }: { onSwitchToPos: () => void, onCancelAdisyon?: () => void }) {
    ...
    const [cancelView, setCancelView] = useState<'MAIN' | 'SUBCHECK' | 'ITEM' | 'ITEM_REFUND' | 'SUBCHECK_REFUND'>('MAIN');
    const [cancelReason, setCancelReason] = useState('');
    const canCancelSale = isSuperAdmin || (user?.extraPermissions || []).includes('OP:CAN_CANCEL_SALE');
    ...
    const [tempNameTable, setTempNameTable] = useState<Table | null>(null);
    ...
    // --- Cancellation Logic ---
    const handleCancelAdisyonLocal = async () => {
        if (!canCancelSale) {
            toastSwal({ icon: 'warning', title: 'İptal yetkiniz bulunmamaktadır.' });
            return;
        }
        if (!selectedTable) return;
        setCancelView('MAIN');
    ...
    const confirmCancelTable = async (reason: string) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setExistingOrders([]);
                refreshDynamicData();
                // Opsiyonel: Masa listesine geri dön
                setActiveTab('tables');
    ...
    const confirmRefundSubCheck = async (subCheckId: number, reason: string) => {
        try {
            const token = localStorage.getItem('token') || (user as any)?.token;
            const res = await fetch(`${API_URL}/sales/${subCheckId}/refund`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                body: JSON.stringify({ reason })
            });
            if (res.ok) {
                toastSwal({ icon: 'success', title: 'Adisyon iade edildi.' });
                setIsCancellationModalOpen(false);
                if (selectedTable) fetchTableData(selectedTable.id);
                refreshDynamicData();
            } else {
    ...
    const handleBulkApply = async (type: string, reason?: string) => {
        const cartItemsToUpdate = bulkSelectedItems.filter(b => b.type === 'CART').map(b => b.identifier as number);
        const existingItemsToUpdate = bulkSelectedItems.filter(b => b.type === 'EXISTING').map(b => b.identifier as number);

        if (cartItemsToUpdate.length > 0) {
            setCart(prev => prev.map((item, idx) => {
                if (cartItemsToUpdate.includes(idx)) {
                    return { ...item, transactionType: type as any, transactionReason: reason };
                }
                return item;
            }));
        }
        ...
        if (existingItemsToUpdate.length > 0) {
            try {
                const token = localStorage.getItem('token') || (user as any)?.token;
                await Promise.all(existingItemsToUpdate.map(id =>
        ...
        setIsBulkReasonModalOpen(false);
        setBulkSelectedItems([]);
        toastSwal({ icon: 'success', title: 'Toplu işlem uygulandı' });
    };

    const handleUpdateTempName = async () => {
        if (!tempNameTable) return;
        try {
            const token = localStorage.getItem('token') || (user as any)?.token;
            const res = await fetch(`${API_URL}/tables/${tempNameTable.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                body: JSON.stringify({ tempName: tempNameValue.trim() || null })
            });
            if (res.ok) {
                toastSwal({ icon: 'success', title: 'Masa ismi güncellendi' });
                refreshDynamicData();
                setIsTempNameModalOpen(false);
                setTempNameTable(null);
                setTempNameValue('');
            }
        } catch (error) {
            console.error(error);
            toastSwal({ icon: 'error', title: 'Hata oluştu' });
        }
    };
    ...
    const saveLineDiscount = async () => {
        ...
        if (selectedDiscountItem.type === 'CART') {
            ...
            setCart(prev => {
                const newCart = [...prev];
                const originalItem = newCart[selectedDiscountItem.index];
                if (!originalItem) return prev;

                const basePrice = calculateItemPrice(originalItem);
                const qtyToDiscount = Number(lineDiscountQty || 0);

                if (qtyToDiscount > 0 && qtyToDiscount < originalItem.quantity) {
                    // Split!
                    const remainingQty = originalItem.quantity - qtyToDiscount;
                    // Orijinal satırı güncelle (indirim kaldırılır)
            ...
            try {
                ...
                console.error(e);
                toastSwal({ icon: 'error', title: 'Sunucuyla iletişim kurulurken bir hata oluştu.' });
            }
        }
    };
            ...
            if (activeSubCheckId) {
                ...
                const orderPayload = {
                    ...
                    totalAmount: cart.reduce((sum: number, item: OrderItem) => {
                        ...
                        return sum + base + extras - Number(item.discountAmount || 0);
                    }, 0),
                    status: 'NEW',
                    items: cart.map((item: OrderItem) => ({
                        productId: item.product.id,
    ...
    const paidTotal = paidItems.reduce((sum, i) => sum + (i.quantity * i.unitPrice - Number(i.discountAmount || 0)), 0);
    const remainingTotal = unpaidItems.reduce((sum, i) => sum + (i.quantity * i.unitPrice - Number(i.discountAmount || 0)), 0) + cartTotal;
    ...
    return (
        <div className="flex h-screen bg-slate-50 dark:bg-slate-800 font-sans overflow-hidden transition-colors duration-300 relative">
                                                ...
                                                {zoneTables.map(table => (
                                                    ...
                                                    >
                                                        ...
                                                        <button
                                                            ...
                                                            className="absolute top-3 left-3 w-8 h-8 flex items-center justify-center rounded-xl bg-yellow-50 dark:bg-yellow-500/10 hover:bg-yellow-100 dark:hover:bg-yellow-500/20 text-yellow-600 dark:text-yellow-400 border border-yellow-200 dark:border-yellow-500/30 transition-all opacity-70 hover:opacity-100 z-10"
                                                            title="Masa Transfer"
                                                        >
                                                        ...
                                                        <button
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                setTempNameTable(table);
                                                                setTempNameValue(table.tempName || '');
                                                                setIsTempNameModalOpen(true);
                                                            }}
                                                            className="absolute top-3 right-3 w-8 h-8 flex items-center justify-center rounded-xl bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/30 transition-all opacity-70 hover:opacity-100 z-10"
                                                            title="Geçici İsim Ver"
                                                        >
                                                            <i className="fat fa-pencil text-xs"></i>
                                                        </button>

                                                        <span className="text-2xl mb-1 group-hover:scale-110 transition-transform">
                                                            {table.status === 'BOŞ' ? '🪑' : table.status === 'REZERVE' ? '📅' : '🍽️'}
                                                        </span>
                                                        <span className="font-extrabold text-slate-800 dark:text-white uppercase tracking-tighter text-sm line-clamp-1 px-1">
                                                            {table.tempName ? (
                    ...
                    <div className="p-6 border-b border-slate-100/50 dark:border-slate-700/50">
                        <div className="flex items-center justify-between mb-1">
                            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                                Sipariş Özeti
                                {selectedTable && (
                                    <span className="bg-emerald-100/80 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 py-1.5 px-4 rounded-full text-[10px] uppercase tracking-widest font-black shadow-inner">{selectedTable.name}</span>
                                )}
                            </h2>
                            {(cart.length > 0 || unpaidItems.length > 0) && (
                                <button
                                    onClick={() => {
                                        setBulkSelectedItems([]);
                                        setIsBulkTransactionOpen(true);
                                    }}
                                    className="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border border-indigo-200 dark:border-indigo-500/30 flex items-center gap-1.5 shadow-sm hover:shadow-md active:scale-95"
                                >
                                    <i className="fat fa-list-check"></i> İşlem Tipi
                                </button>
                            )}
                        </div>
                                    ...
                                    <div className={`flex flex-col gap-2 p-3 rounded-2xl border shadow-sm transition-all ${item.status && item.status !== 'ACTIVE' ? `opacity-60 grayscale line-through italic bg-slate-200/50 dark:bg-slate-800/50 border-slate-300 dark:border-slate-700 ${item.status === 'REFUNDED' ? 'text-rose-500 dark:text-rose-400' : ''}` : item.isPaid ? 'bg-emerald-50/60 dark:bg-emerald-900/10 border-emerald-200/50 dark:border-emerald-500/20 opacity-70' : 'bg-slate-100/50 dark:bg-slate-700/30 border-slate-200/50 dark:border-slate-700/50'}`}>
                                        <div className="flex justify-between items-start">
                                            ...
                                            <span className={`font-bold ${item.isPaid ? 'text-emerald-600 dark:text-emerald-400 line-through' : 'text-slate-600 dark:text-slate-300'}`}>
                                                ...
                                                ₺{((item.quantity * item.unitPrice) - Number(item.discountAmount || 0)).toFixed(2)}
                                            </span>
                                        </div>
                                ...
                                <div key={`new-${index}`} className="flex flex-col gap-2 p-3 bg-white/50 dark:bg-slate-800/50 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 shadow-sm">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <span className="font-bold text-slate-800 dark:text-slate-100">
                                            ...
                                            ₺{((calculateItemPrice(item) * item.quantity) - Number(item.discountAmount || 0)).toFixed(2)}
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center mt-1">
                                        <div className="flex flex-col gap-1 w-full max-w-[150px]">
                                            <span className="text-xs font-bold text-slate-400">
                                                Birim: ₺{calculateItemPrice(item).toFixed(2)}
                                                {Number(item.discountAmount || 0) > 0 && (
                                                    <span className="text-red-500 dark:text-red-400 ml-1">
                                                        (-₺{Number(item.discountAmount || 0).toFixed(2)})
                                                    </span>
                    ...
                    <div className="p-6 bg-slate-100/60 dark:bg-slate-800/60 backdrop-blur-xl border-t border-white/50 dark:border-slate-700/50 m-4 rounded-3xl transition-colors shadow-inner">
                        ...
                        <div className="flex gap-2 w-full">
                            <button
                                ...
                                className="w-1/4 py-3 rounded-2xl border-2 border-slate-200 dark:border-slate-600 font-bold text-slate-600 dark:text-slate-300 disabled:opacity-50"
                            >
                                Temizle
                            </button>
                            <button
                                onClick={handleCancelAdisyonLocal}
                                disabled={!canCancelSale || !selectedTable || (cart.length === 0 && allFlatChecks.length === 0)}
                                className="w-1/4 py-3 rounded-2xl bg-rose-50 dark:bg-rose-500/10 border-2 border-rose-200 dark:border-rose-500/30 text-rose-700 dark:text-rose-400 font-bold disabled:opacity-50 flex items-center justify-center transition-all active:scale-95"
                                title="Adisyon İptal İşlemleri"
                            >
                                <i className="fat fa-trash"></i>
                            </button>
                            <button
                                ...
                                className="flex-1 py-3 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-bold shadow-lg shadow-emerald-500/30 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center gap-2"
                            >
                                {isSending ? (
                                    <>
            ...
            {/* --- Toplu İşlem Modalı --- */}
            {isBulkTransactionOpen && (
                <div className="fixed inset-0 z-[250] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="w-full max-w-2xl bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl overflow-hidden border border-white/20 dark:border-slate-700/50 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
                        <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-indigo-50 dark:bg-indigo-900/10">
                            <div className="flex items-center gap-3">
                                <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 rounded-2xl flex items-center justify-center shadow-inner">
                                    <i className="fat fa-list-check text-xl"></i>
                                </div>
                                <div>
                                    <h2 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight">Toplu İşlem</h2>
                                    <p className="text-xs font-bold text-slate-500">İşlem tipini değiştirmek istediğiniz ürünleri seçin.</p>
                                </div>
                            </div>
                            <button onClick={() => setIsBulkTransactionOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-xl bg-white dark:bg-slate-700 text-slate-400 hover:text-rose-500 shadow-sm transition-colors">
                                <i className="fat fa-xmark text-lg"></i>
                            </button>
                        </div>

                        <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar bg-slate-50/50 dark:bg-slate-900/20">
                            {unpaidItems.length > 0 && (
                                <div className="space-y-2">
                                    <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest flex items-center justify-between">
                                        ...
                                        <button
                                            onClick={() => {
                                                const allSelected = unpaidItems.every(i => bulkSelectedItems.some(b => b.type === 'EXISTING' && b.identifier === i.id));
                                                if (allSelected) {
                                                    setBulkSelectedItems(prev => prev.filter(b => b.type !== 'EXISTING'));
                                                } else {
                                                    const existingSelections = bulkSelectedItems.filter(b => b.type === 'CART');
                                                    setBulkSelectedItems([...existingSelections, ...unpaidItems.map(i => ({ type: 'EXISTING' as const, identifier: i.id }))]);
                                                }
                                            }}
                                            className="text-[10px] bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-3 py-1.5 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors"
                                        >
                                            Tümünü Seç / Bırak
                                        </button>
                                    </h3>
                                    {unpaidItems.map((item: any) => (
                                        <label key={`ex-${item.id}`} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${bulkSelectedItems.some(b => b.type === 'EXISTING' && b.identifier === item.id) ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-500/10' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800'}`}>
                                            <input
                                                type="checkbox"
                                                className="w-5 h-5 rounded text-indigo-600 focus:ring-indigo-500 dark:bg-slate-700 dark:border-slate-600"
                                                checked={bulkSelectedItems.some(b => b.type === 'EXISTING' && b.identifier === item.id)}
                                                onChange={(e) => {
                                                    if (e.target.checked) setBulkSelectedItems(prev => [...prev, { type: 'EXISTING', identifier: item.id }]);
                                                    else setBulkSelectedItems(prev => prev.filter(b => !(b.type === 'EXISTING' && b.identifier === item.id)));
                                                }}
                                            />
                                            <div className="flex-1">
                                                <div className="font-bold text-slate-800 dark:text-white text-sm">{item.product.name} <span className="opacity-50 font-normal">x{item.quantity}</span></div>
                                                <div className="text-[10px] text-slate-500 mt-0.5">Mevcut Tip: {item.transactionType || 'SALE'}</div>
                                            </div>
                                            <div className="font-black text-slate-800 dark:text-slate-200">₺{(item.quantity * item.unitPrice).toFixed(2)}</div>
                                        </label>
                                    ))}
                                </div>
                            )}
                            ...
                            {cart.length > 0 && (
                                <div className="space-y-2 mt-4">
                                    <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest flex items-center justify-between">
                                        Sepetteki Ürünler (Yeni)
                                        <button
                                            onClick={() => {
                                                const allSelected = cart.every((_, idx) => bulkSelectedItems.some(b => b.type === 'CART' && b.identifier === idx));
                                                if (allSelected) {
                                                    setBulkSelectedItems(prev => prev.filter(b => b.type !== 'CART'));
                                                } else {
                                                    const cartSelections = cart.map((_, idx) => ({ type: 'CART', identifier: idx } as { type: 'CART' | 'EXISTING', identifier: any }));
                                                    const existingSelections = bulkSelectedItems.filter(b => b.type === 'EXISTING');
                                                    setBulkSelectedItems([...existingSelections, ...cartSelections]);
                                                }
                                            }}
                                            className="text-[10px] bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-3 py-1.5 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors"
                                        >
                                    ...
                                    {cart.map((item, idx) => (
                                        <label key={`cart-${idx}`} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${bulkSelectedItems.some(b => b.type === 'CART' && b.identifier === idx) ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-500/10' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800'}`}>
                                            <input
                                                type="checkbox"
                                                className="w-5 h-5 rounded text-indigo-600 focus:ring-indigo-500 dark:bg-slate-700 dark:border-slate-600"
                                                checked={bulkSelectedItems.some(b => b.type === 'CART' && b.identifier === idx)}
                                                onChange={(e) => {
                                                    if (e.target.checked) setBulkSelectedItems(prev => [...prev, { type: 'CART', identifier: idx }]);
                                                    else setBulkSelectedItems(prev => prev.filter(b => !(b.type === 'CART' && b.identifier === idx)));
                                                }}
                                            />
                                            <div className="flex-1">
                                                <div className="font-bold text-slate-800 dark:text-white text-sm">{item.product.name} {item.variationName ? `(${item.variationName})` : ''} <span className="opacity-50 font-normal">x{item.quantity}</span></div>
                                                <div className="text-[10px] text-slate-500 mt-0.5">Mevcut Tip: {item.transactionType || 'SALE'}</div>
                                            </div>
                                            <div className="font-black text-slate-800 dark:text-slate-200">₺{(calculateItemPrice(item) * item.quantity).toFixed(2)}</div>
                                        </label>
                                    ))}
                                </div>
                            )}
                            ...
                            {cart.length === 0 && unpaidItems.length === 0 && (
                                <div className="text-center p-8 text-slate-400">
                                    <i className="fat fa-box-open text-4xl mb-3 opacity-50"></i>
                                    <p>İşlem yapılabilecek ürün bulunamadı.</p>
                                </div>
                            )}
                        </div>
                        ...
                        {bulkSelectedItems.length > 0 && (
                            <div className="p-6 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Seçilen <span className="text-indigo-500">{bulkSelectedItems.length}</span> ürüne uygulanacak işlem tipi:</p>
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
                                    <button onClick={() => setBulkTransactionType('SALE')} className={`py-3 px-2 rounded-xl text-xs font-bold transition-all border-2 flex flex-col items-center gap-1 ${bulkTransactionType === 'SALE' ? 'border-emerald-500 bg-emerald-50 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-emerald-300'}`}>
                                        <i className="fat fa-receipt text-lg"></i> Satış
                                    </button>
                                    {(isSuperAdmin || (user?.extraPermissions || []).includes('OP:CAN_COMPLIMENTARY')) && (
                                        <button onClick={() => setBulkTransactionType('COMPLIMENTARY')} className={`py-3 px-2 rounded-xl text-xs font-bold transition-all border-2 flex flex-col items-center gap-1 ${bulkTransactionType === 'COMPLIMENTARY' ? 'border-indigo-500 bg-indigo-50 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-indigo-300'}`}>
                                            <i className="fat fa-gift text-lg"></i> İkram
                                        </button>
                                    )}
                                    {(isSuperAdmin || (user?.extraPermissions || []).includes('OP:NON_PAYMENT')) && (
                                        <button onClick={() => setBulkTransactionType('FREE')} className={`py-3 px-2 rounded-xl text-xs font-bold transition-all border-2 flex flex-col items-center gap-1 ${bulkTransactionType === 'FREE' ? 'border-rose-500 bg-rose-50 text-rose-600 dark:bg-rose-500/20 dark:text-rose-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-rose-300'}`}>
                                            <i className="fat fa-hand-holding-heart text-lg"></i> Ödenmez
                                        </button>
                                    )}
                                    {(isSuperAdmin || (user?.extraPermissions || []).includes('OP:CAN_PROMOTION')) && (
                                        <button onClick={() => setBulkTransactionType('PROMOTION')} className={`py-3 px-2 rounded-xl text-xs font-bold transition-all border-2 flex flex-col items-center gap-1 ${bulkTransactionType === 'PROMOTION' ? 'border-teal-500 bg-teal-50 text-teal-600 dark:bg-teal-500/20 dark:text-teal-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-teal-300'}`}>
                                            <i className="fat fa-percentage text-lg"></i> Promo
                                        </button>
                                    )}
                                    {(isSuperAdmin || (user?.extraPermissions || []).includes('OP:STAFF_SALE')) && (
                                        <button onClick={() => setBulkTransactionType('STAFF')} className={`py-3 px-2 rounded-xl text-xs font-bold transition-all border-2 flex flex-col items-center gap-1 ${bulkTransactionType === 'STAFF' ? 'border-amber-500 bg-amber-50 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-amber-300'}`}>
                                            <i className="fat fa-user-tie text-lg"></i> Personel
                                        </button>
                                    )}
                                    {(isSuperAdmin || (user?.extraPermissions || []).includes('OP:CAN_TICKET')) && (
                                        <button onClick={() => setBulkTransactionType('TICKET')} className={`py-3 px-2 rounded-xl text-xs font-bold transition-all border-2 flex flex-col items-center gap-1 ${bulkTransactionType === 'TICKET' ? 'border-blue-500 bg-blue-50 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-blue-300'}`}>
                                            <i className="fat fa-ticket text-lg"></i> Bilet
                                        </button>
                                    )}
                                </div>
                                <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-end">
                                    <button
                                        ...
                                        className="px-8 py-4 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase tracking-widest shadow-xl shadow-indigo-500/30 transition-all hover:scale-[1.02] active:scale-95 flex items-center gap-2"
                                    >
                                        Seçilenlere Uygula <i className="fat fa-check-circle"></i>
                                    </button>
                                </div>
                            </div>
                        )}
            ...
            {/* --- Toplu İşlem Zorunlu Sebep Seçim Modalı --- */}
            {isBulkReasonModalOpen && (
                <div className="fixed inset-0 z-[260] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl overflow-hidden border border-white/20 dark:border-slate-700/50 p-8 animate-in fade-in zoom-in duration-300">
                        ...
                        <div className="flex gap-4">
                            <button
                                onClick={() => setIsBulkReasonModalOpen(false)}
                                className="flex-1 py-4 rounded-2xl bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-black text-xs uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-600 transition-all"
                            >
                                Vazgeç
                            </button>
            ...
            {/* --- Geçici İsim Verme Modalı --- */}
            {isTempNameModalOpen && (
                <div className="fixed inset-0 z-[300] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl overflow-hidden border border-white/20 dark:border-slate-700/50 p-8 animate-in fade-in zoom-in duration-300">
                        <div className="text-center mb-8">
                            <div className="w-20 h-20 rounded-full bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-indigo-500/20">
                                <i className="fat fa-pencil text-3xl"></i>
                            </div>
                            <h2 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tight">Geçici İsim Ver</h2>
                            <p className="text-slate-500 dark:text-slate-400 text-sm mt-2 font-medium">{tempNameTable?.name} için tanımlayıcı bir isim girin.</p>
                        </div>
                        ...
                        <div className="space-y-4 mb-8">
                            <div className="relative">
                                <input
                                    type="text"
                                    autoFocus
                                    placeholder="Örn: Ahmet Bey, Kırmızı Tişörtlü..."
                                    value={tempNameValue}
                                    onChange={(e) => setTempNameValue(e.target.value)}
                                    onKeyDown={(e) => e.key === 'Enter' && handleUpdateTempName()}
                                    className="w-full p-5 bg-slate-50 dark:bg-slate-900/50 border-2 border-slate-100 dark:border-slate-700 rounded-2xl focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all text-lg font-bold text-slate-800 dark:text-white placeholder:text-slate-400"
                                />
                                {tempNameValue && (
                                    <button
                                        onClick={() => setTempNameValue('')}
                                        className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-rose-500 transition-colors"
                                    >
                                        <i className="fat fa-circle-xmark text-xl"></i>
                                    </button>
                                )}
                            </div>

                            <div className="flex flex-wrap gap-2">
                                {['Vip', 'Müşteri', 'Personel', 'Rezervasyon', 'Grup'].map(tag => (
                                    <button
                                        key={tag}
                                        onClick={() => setTempNameValue(tag)}
                                        className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-[10px] font-bold uppercase hover:bg-indigo-500 hover:text-white transition-all"
                                    >
                                        {tag}
                                    </button>
                                ))}
                        ...
                        <div className="flex gap-4">
                            <button
                                onClick={() => { setIsTempNameModalOpen(false); setTempNameTable(null); setTempNameValue(''); }}
                                className="flex-1 py-4 rounded-2xl bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-black text-xs uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-600 transition-all"
                            >
                                Vazgeç
                            </button>
                            <button
                                onClick={handleUpdateTempName}
                                className="flex-1 py-4 rounded-2xl bg-indigo-600 text-white font-black text-xs uppercase tracking-widest hover:bg-indigo-500 shadow-xl shadow-indigo-500/30 transition-all active:scale-95"
                            >
                                Kaydet
                            </button>
                        </div>
            ...
            {isCancellationModalOpen && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-white dark:bg-slate-800 rounded-[40px] w-full max-w-2xl shadow-[0_32px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden border border-white/20 dark:border-slate-700/50 flex flex-col max-h-[90vh] animate-in zoom-in duration-300">
                        {/* Header */}
                        <div className="p-8 shrink-0 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 ${cancelView === 'ITEM_REFUND' || cancelView === 'SUBCHECK_REFUND' ? 'bg-indigo-500/20 text-indigo-500' : 'bg-rose-500/20 text-rose-500'} rounded-2xl flex items-center justify-center transition-colors`}>
                                    <i className={`fat ${cancelView === 'ITEM_REFUND' || cancelView === 'SUBCHECK_REFUND' ? 'fa-rotate-left' : 'fa-trash-can'} text-2xl`}></i>
                                </div>
                                <div>
                                    <h2 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tight">
                                        {cancelView?.includes('REFUND') ? 'İade İşlemleri' : 'İptal İşlemleri'}
                                    </h2>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                            {cancelView === 'MAIN' && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {/* IPTAL GRUBU */}
                                    <div className="space-y-4">
                                        <h3 className="text-xs font-black text-rose-500 uppercase tracking-widest px-2">İptal İşlemleri</h3>
                                        <button
                                            onClick={async () => {
                                                const Swal = (await import('sweetalert2')).default;
                                            ...
                                            className="w-full group p-6 rounded-[24px] bg-rose-500 hover:bg-rose-600 transition-all shadow-xl shadow-rose-500/20 flex items-center gap-6 active:scale-95"
                                        >
                                            <div className="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
                                                <i className="fat fa-table-list text-2xl text-white"></i>
                                            </div>
                                        ...
                                        <button
                                            ...
                                            className="w-full group p-6 rounded-[24px] bg-white dark:bg-slate-700 border-2 border-slate-100 dark:border-slate-600 hover:border-rose-500 transition-all shadow-lg flex items-center gap-6 active:scale-95"
                                        >
                                            <div className="w-12 h-12 rounded-xl bg-rose-50 flex items-center justify-center shrink-0">
                                                <i className="fat fa-receipt text-2xl text-rose-500"></i>
                                            </div>
                                        ...
                                        >
                                            <div className="w-12 h-12 rounded-xl bg-rose-50 flex items-center justify-center shrink-0">
                                                <i className="fat fa-burger-soda text-2xl text-rose-500"></i>
                                            </div>
                                    ...
                                    {/* IADE GRUBU */}
                                    <div className="space-y-4">
                                        <h3 className="text-xs font-black text-indigo-500 uppercase tracking-widest px-2">İade İşlemleri</h3>
                                        <button
                                            onClick={() => setCancelView('SUBCHECK_REFUND')}
                                            className="w-full group p-6 rounded-[24px] bg-indigo-500 hover:bg-indigo-600 transition-all shadow-xl shadow-indigo-500/20 flex items-center gap-6 active:scale-95"
                                        >
                                            <div className="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
                                                <i className="fat fa-receipt text-2xl text-white"></i>
                                            </div>
                                            <span className="text-lg font-black text-white uppercase tracking-tight">Adisyon İade Et</span>
                                        </button>

                                        <button
                                            onClick={() => setCancelView('ITEM_REFUND')}
                                            className="w-full group p-6 rounded-[24px] bg-white dark:bg-slate-700 border-2 border-slate-100 dark:border-slate-600 hover:border-indigo-500 transition-all shadow-lg flex items-center gap-6 active:scale-95"
                                        >
                                            <div className="w-12 h-12 rounded-xl bg-indigo-50 flex items-center justify-center shrink-0">
                                                <i className="fat fa-burger-soda text-2xl text-indigo-500"></i>
                                            </div>
                                            <span className="text-lg font-black text-slate-800 dark:text-white uppercase tracking-tight">Ürün İade Et</span>
                                        </button>
                                    </div>
                                </div>
                            )}

                            {(cancelView === 'SUBCHECK' || cancelView === 'SUBCHECK_REFUND') && (
                                <div className="space-y-4">
                                    ...
                                    {allFlatChecks.map((check: any) => (
                                        <div key={check.id} className={`p-6 rounded-3xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between group hover:border-${cancelView === 'SUBCHECK_REFUND' ? 'indigo' : 'rose'}-500/50 transition-all`}>
                                            <div>
                                            ...
                                            <button
                                                onClick={async () => {
                                                    const Swal = (await import('sweetalert2')).default;
                                                    const isRefund = cancelView === 'SUBCHECK_REFUND';
                                                    const { value: reason } = await Swal.fire({
                                                    ...
                                                    if (reason !== undefined) {
                                                        if (isRefund) confirmRefundSubCheck(check.id, reason);
                                                        else confirmCancelSubCheck(check.id, reason);
                                                    }
                                                }}
                                                className={`px-6 py-3 rounded-xl ${cancelView === 'SUBCHECK_REFUND' ? 'bg-indigo-500 shadow-indigo-500/20' : 'bg-rose-500 shadow-rose-500/20'} text-white font-black text-xs uppercase tracking-widest shadow-lg active:scale-95 transition-all`}
                                            >
                                                {cancelView === 'SUBCHECK_REFUND' ? 'İade Et' : 'İptal Et'}
                                            </button>
                                        </div>
                                    ))}
                                </div>
                            ...
                            {(cancelView === 'ITEM' || cancelView === 'ITEM_REFUND') && (
                                <div className="space-y-4">
                                    <button onClick={() => setCancelView('MAIN')} className="text-slate-400 hover:text-indigo-500 font-bold text-sm mb-4 flex items-center gap-2 transition-colors">
                                        <i className="fat fa-arrow-left"></i> Geri Dön
                                    </button>
                                    <div className="grid grid-cols-1 gap-3">
                                        {allFlatChecks.flatMap((c: any) => (c.items || [])).filter((i: any) => i.status === 'ACTIVE').map((item: any) => (
                                            <div key={item.id} className={`p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between group hover:border-${cancelView?.includes('REFUND') ? 'indigo' : 'rose'}-500/50 transition-all`}>
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-10 h-10 bg-white dark:bg-slate-800 rounded-xl flex items-center justify-center font-black ${cancelView?.includes('REFUND') ? 'text-indigo-500' : 'text-rose-500'} border border-slate-100 dark:border-slate-700`}>
                                                        {item.quantity}
                                                    </div>
                                                    <div>
                                                ...
                                                >
                                                    <i className={`fat ${cancelView?.includes('REFUND') ? 'fa-rotate-left' : 'fa-trash-can'}`}></i>
                                                </button>
                                            </div>
                                        ))}
                                        {allFlatChecks.flatMap((c: any) => (c.items || [])).filter((i: any) => i.status === 'ACTIVE').length === 0 && (
                                            <div className="text-center py-12">
                                                <i className="fat fa-circle-info text-4xl text-slate-300 mb-4 block"></i>
                                                <p className="text-slate-400 font-bold">Burada görüntülenecek ürün bulunamadı.</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
            ...
            {/* Satır İndirimi (Line Item Discount) Modal */}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-md shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] overflow-hidden border border-white/20 dark:border-slate-700/50 flex flex-col scale-100 animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6 space-y-4">
                            {selectedDiscountItem.quantity > 1 && (
                                <div className="flex flex-col gap-1 bg-slate-50 dark:bg-slate-900/50 px-4 py-3 rounded-2xl border border-slate-100 dark:border-slate-700/30">
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Toplam Satır Adedi</span>
                            ...
                            <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-900/50 px-4 py-3 rounded-2xl border border-slate-100 dark:border-slate-700/30">
                                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                                    {selectedDiscountItem.quantity > 1 ? 'Uygulanacak Kısım Toplamı' : 'Satır Toplamı (Birim x Adet)'}
                                </span>
                                <span className="text-sm font-extrabold text-slate-700 dark:text-slate-200 font-mono">
                                    {(() => {
                                        const basePrice = selectedDiscountItem.type === 'CART' 
                                            ? calculateItemPrice(selectedDiscountItem) 
                                            : (selectedDiscountItem.unitPrice ?? selectedDiscountItem.product?.price ?? 0);
                                        return `₺${(basePrice * lineDiscountQty).toFixed(2)}`;
                                    })()}
                                </span>
                            </div>
                        ...
                        <div className="p-6 shrink-0 border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 flex gap-3">
                            <button
                                ...
                                className="flex-1 py-3 rounded-2xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 font-bold text-sm shadow-sm transition-all hover:bg-slate-50 dark:hover:bg-slate-650 active:scale-[0.98]"
                            >
                                Vazgeç
                            </button>
                            <button
