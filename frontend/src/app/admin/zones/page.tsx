...
export default function ZonesAdminPage() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (formData.id === 0) {
                ...
                toastSwal({ title: 'Başarılı!', text: 'Bölüm eklendi.', icon: 'success' });
            } else {
                await axios.put(`http://localhost:3050/zones/${formData.id}`, payload, config);
                toastSwal({ title: 'Başarılı!', text: 'Bölüm güncellendi.', icon: 'success' });
            }
            setIsModalOpen(false);
            fetchData();
        } catch (error: any) {
            console.error('Error saving zone', error);
            showSwal({ title: 'Hata', text: error?.response?.data?.message || 'Bölüm kaydedilemedi.', icon: 'error' });
        }
    };
    ...
    const handleDelete = async (id: number) => {
        const result = await showSwal({
            title: 'Emin misiniz?',
            text: "Bu bölümü silmek istediğinize emin misiniz?",
            icon: 'warning',
        ...
        if (result.isConfirmed && user?.token) {
            try {
                ...
                toastSwal({ title: 'Silindi!', text: 'Bölüm başarıyla silindi.', icon: 'success' });
                fetchData();
            } catch (error) {
                console.error('Error deleting zone', error);
                showSwal({ title: 'Hata', text: 'Bölüm silinirken bir sorun oluştu.', icon: 'error' });
            }
        }
    };
            ...
            <div className="w-full px-[50px] py-8 relative z-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div className="flex items-center">
                        ...
                        <div>
                            <h3 className="mb-0 text-3xl font-extralight text-indigo-600 dark:text-indigo-400 leading-none uppercase tracking-[0.25em]">Kat/Bölüm Yönetimi</h3>
                            <h5 className="text-muted mb-0 text-lg font-medium text-slate-400 dark:text-slate-500 mt-0.5">Şubelerinize ait salon, teras veya bölümleri düzenleyin.</h5>
                        </div>
                    </div>
                    <div className="flex gap-3">
                        <button onClick={() => openModal()} className="px-6 py-3 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 text-indigo-600 dark:text-indigo-400 font-black text-xs uppercase tracking-widest rounded-2xl shadow-sm hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-all flex items-center gap-2 hover:scale-105 active:scale-95">
                            <i className="fat fa-plus-circle text-lg"></i> Yeni Bölüm
                        </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl p-6 rounded-[32px] border border-white dark:border-slate-700 flex items-center justify-between transition-all hover:border-indigo-300 dark:hover:border-indigo-500/40 hover:shadow-[0_8px_30px_-5px_rgba(99,102,241,0.3)] hover:scale-[1.02] cursor-pointer">
                        <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Toplam Bölüm</p>
                    ...
                    <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl p-6 rounded-[32px] border border-white dark:border-slate-700 flex items-center justify-between transition-all hover:border-emerald-300 dark:hover:border-emerald-500/40 hover:shadow-[0_8px_30px_-5px_rgba(16,185,129,0.3)] hover:scale-[1.02] cursor-pointer">
                        <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Aktif Şube Sayısı</p>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead className="sticky top-0 z-10">
                                    <tr className="bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-700/50">
                                        ...
                                        <th className="px-8 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Bölüm Adı</th>
                                        ...
                                        <tr key={z.id} className="hover:bg-indigo-500/5 dark:hover:bg-indigo-500/10 transition-all group">
                                            ...
                                            <td className="px-8 py-3">
                                                <p className="text-sm font-bold text-slate-600 dark:text-slate-400 line-clamp-2 max-w-xs">
                                                    {z.description || 'Açıklama girilmemiş.'}
                                                </p>
                                            </td>
                                        ...
                                        <tr>
                                            <td colSpan={5} className="p-20 text-center">
                                                <div className="flex flex-col items-center opacity-40">
                                                    ...
                                                    <p className="text-slate-500 font-bold uppercase tracking-widest text-sm">Kayıtlı Bölüm Bulunamadı</p>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
                        ...
                        <div className="p-8 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/20">
                            <div>
                                <h2 className="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-3 tracking-tighter uppercase">
                                    ...
                                    {formData.id === 0 ? 'YENİ BÖLÜM' : 'BÖLÜM DÜZENLE'}
                                </h2>
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Bölüm bilgilerini ve şube atamasını yapın</p>
                            </div>
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-6">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-800 dark:text-white font-bold focus:ring-4 focus:ring-indigo-500/10 outline-none transition-shadow" rows={3} placeholder="Bölüm hakkında kısa bilgi..."></textarea>
                                </div>
                            </div>
                            <div className="pt-6 flex flex-col gap-3">
                                <button type="submit" className="w-full py-4 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-[24px] font-black text-sm uppercase tracking-widest shadow-md shadow-indigo-500/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50" disabled={formData.locationId === 0}>
                                    BÖLÜMÜ KAYDET
                                </button>
