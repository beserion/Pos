...
export default function LocationsAdminPage() {
    ...
    const fetchLocations = async () => {
        ...
        try {
            ...
            showSwal({ title: 'Hata', text: 'Şubeler yüklenirken bir sorun oluştu.', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (formData.id === 0) {
                ...
                toastSwal({ title: 'Başarılı!', text: 'Şube eklendi.', icon: 'success' });
            } else {
                await axios.put(`http://localhost:3050/locations/${formData.id}`, formData, config);
                toastSwal({ title: 'Başarılı!', text: 'Şube güncellendi.', icon: 'success' });
            }
            setIsModalOpen(false);
            fetchLocations();
        } catch (error: any) {
            console.error('Error saving location', error);
            showSwal({ title: 'Hata', text: error?.response?.data?.message || 'Şube kaydedilemedi.', icon: 'error' });
        }
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        if (result.isConfirmed && user?.token) {
            try {
                ...
                toastSwal({ title: 'Silindi!', text: 'Şube başarıyla silindi.', icon: 'success' });
                fetchLocations();
            } catch (error) {
                console.error('Error deleting location', error);
                showSwal({ title: 'Hata', text: 'Şube silinirken bir sorun oluştu.', icon: 'error' });
            }
        }
    };
            ...
            <div className="w-full px-[50px] py-8 relative z-10">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div className="flex items-center">
                        ...
                        <div>
                            <h3 className="mb-0 text-3xl font-extralight text-blue-600 dark:text-blue-400 leading-none uppercase tracking-[0.25em]">ŞUBE YÖNETİMİ</h3>
                            <h5 className="text-muted mb-0 text-lg font-medium text-slate-400 dark:text-slate-500 mt-0.5">İşletme şube ve merkezlerini yönetin.</h5>
                        </div>
                    </div>
                    <div className="flex gap-3">
                        <button onClick={() => openModal()} className="px-6 py-3 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 text-blue-600 dark:text-blue-400 font-black text-xs uppercase tracking-widest rounded-2xl shadow-sm hover:bg-blue-100 dark:hover:bg-blue-500/20 transition-all flex items-center gap-2 hover:scale-105 active:scale-95">
                            <i className="fat fa-plus-circle text-lg"></i> Yeni Lokasyon
                        </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl p-6 rounded-[32px] border border-white dark:border-slate-700 flex items-center justify-between transition-all hover:border-indigo-300 dark:hover:border-indigo-500/40 hover:shadow-[0_8px_30px_-5px_rgba(99,102,241,0.3)] hover:scale-[1.02] cursor-pointer">
                        <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Toplam Lokasyon</p>
                    ...
                    <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl p-6 rounded-[32px] border border-white dark:border-slate-700 flex items-center justify-between transition-all hover:border-emerald-300 dark:hover:border-emerald-500/40 hover:shadow-[0_8px_30px_-5px_rgba(16,185,129,0.3)] hover:scale-[1.02] cursor-pointer">
                        <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Aktif Şubeler</p>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead className="sticky top-0 z-10">
                                    <tr className="bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-700/50">
                                        ...
                                        <th className="px-8 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Şube Bilgisi</th>
                                        <th className="px-8 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">İletişim</th>
                                        <th className="px-8 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Adres</th>
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className={`text-[10px] font-bold mt-1.5 uppercase tracking-widest ${loc.isActive ? 'text-emerald-500' : 'text-red-500'}`}>
                                                            {loc.isActive ? 'AKTİF LOKASYON' : 'PASİF'}
                                                        </p>
                                                    </div>
                                                </div>
                                            </td>
                                        ...
                                        <tr>
                                            <td colSpan={5} className="p-20 text-center">
                                                <div className="flex flex-col items-center opacity-40">
                                                    ...
                                                    <p className="text-slate-500 font-bold uppercase tracking-widest text-sm">Kayıtlı Lokasyon Bulunamadı</p>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-xl">
                    <div className="bg-white dark:bg-slate-800 rounded-[40px] w-full max-w-xl shadow-lg overflow-hidden border border-white/20 dark:border-slate-700/50 animate-in fade-in zoom-in duration-300">
                        <div className="p-8 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/20">
                            <div>
                                <h2 className="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-3 tracking-tighter uppercase">
                                    ...
                                    {formData.id === 0 ? 'YENİ LOKASYON' : 'LOKASYON DÜZENLE'}
                                </h2>
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Şube bilgilerini eksiksiz doldurun</p>
                            </div>
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-6">
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Şube Adı</label>
                                <div className="relative">
                                    <i className="fat fa-tag absolute left-4 top-3.5 text-blue-500/50"></i>
                                    <input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-800 dark:text-white font-bold focus:ring-4 focus:ring-blue-500/10 outline-none transition-shadow" placeholder="Örn: Kadıköy Şubesi" />
                                </div>
                            </div>
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Telefon</label>
                                <div className="relative">
                                    <i className="fat fa-phone absolute left-4 top-3.5 text-blue-500/50"></i>
                                    <input type="text" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-800 dark:text-white font-bold focus:ring-4 focus:ring-blue-500/10 outline-none transition-shadow" placeholder="0212 ..." />
                                </div>
                            </div>
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Açık Adres</label>
                                <div className="relative">
                                    <i className="fat fa-map-location-dot absolute left-4 top-4 text-blue-500/50"></i>
                                    <textarea value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-800 dark:text-white font-bold focus:ring-4 focus:ring-blue-500/10 outline-none transition-shadow" rows={3} placeholder="Mahalle, Cadde, Sokak..."></textarea>
                                </div>
                            </div>
                            <div className="pt-6 flex flex-col gap-3">
                                <button type="submit" className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-[24px] font-black text-sm uppercase tracking-widest shadow-md shadow-blue-500/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2">
                                    <i className="fat fa-save"></i> LOKASYONU KAYDET
                                </button>
