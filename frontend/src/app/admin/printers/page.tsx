...
export default function PrintersAdminPage() {
    ...
    const [formData, setFormData] = useState({ id: 0, name: '', ipAddress: '', isActive: true });
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (formData.id === 0) {
                ...
                await axios.put(`http://localhost:3050/printers/${formData.id}`, formData, config);
    ...
    const openModal = (printer?: Printer) => {
        ...
        else setFormData({ id: 0, name: '', ipAddress: '', isActive: true });
        setIsModalOpen(true);
    };
            ...
            <div className="w-full px-[50px] py-8 relative z-10">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div className="flex items-center">
                        ...
                        <div>
                            <h3 className="mb-0 text-3xl font-extralight text-sky-600 dark:text-sky-400 leading-none uppercase tracking-[0.25em]">YAZICI YÖNETİMİ</h3>
                            <h5 className="text-muted mb-0 text-lg font-medium text-slate-400 dark:text-slate-500 mt-0.5">Mutfak, bar ve adisyon yazıcılarını tanımlayın.</h5>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-[40px] w-full max-w-xl shadow-lg overflow-hidden border border-white/20 dark:border-slate-700/50 animate-in fade-in zoom-in duration-300">
                        <div className="p-8 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/20">
                            <div>
                                ...
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Yazıcı ağ bilgilerini doldurun</p>
                            </div>
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-6">
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Yazıcı Adı / Konumu</label>
                                <div className="relative">
                                    <i className="fat fa-tag absolute left-4 top-3.5 text-sky-500/50"></i>
                                    <input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-800 dark:text-white font-bold focus:ring-4 focus:ring-sky-500/10 outline-none transition-shadow" placeholder="Örn: Mutfak Sıcak, Bar..." />
                                </div>
                            </div>
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">IP Adresi veya Bağlantı Noktası</label>
                                <div className="relative">
                                    <i className="fat fa-network-wired absolute left-4 top-3.5 text-sky-500/50"></i>
                                    <input type="text" value={formData.ipAddress} onChange={(e) => setFormData({ ...formData, ipAddress: e.target.value })} className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-800 dark:text-white font-bold focus:ring-4 focus:ring-sky-500/10 outline-none transition-shadow" placeholder="Örn: 192.168.1.100 veya LPT1" />
                                </div>
                            </div>

                            <div className="pt-6 flex flex-col gap-3">
                                <button type="submit" className="w-full py-4 bg-sky-600 hover:bg-sky-700 text-white rounded-[24px] font-black text-sm uppercase tracking-widest shadow-md shadow-sky-500/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2">
                                    <i className="fat fa-save"></i> YAZICIYI KAYDET
                                </button>
