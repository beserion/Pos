...
export default function IngredientsAdminPage() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (formData.id === 0) {
                ...
                toastSwal({ title: 'Başarılı!', text: 'Hammadde eklendi.', icon: 'success' });
            } else {
                await axios.put(`http://localhost:3050/products/${formData.id}`, payload, config);
                toastSwal({ title: 'Başarılı!', text: 'Hammadde güncellendi.', icon: 'success' });
            }
            setIsModalOpen(false);
            fetchData();
        } catch (error: any) {
            console.error('Error saving ingredient', error);
            showSwal({ title: 'Hata', text: error?.response?.data?.message || 'Hammadde kaydedilemedi.', icon: 'error' });
        }
    };
    ...
    const handleDelete = async (id: number) => {
        const result = await showSwal({
            title: 'Emin misiniz?',
            text: "Bu hammaddeyi silmek istediğinize emin misiniz?",
            icon: 'warning',
        ...
        if (result.isConfirmed && user?.token) {
            try {
                ...
                toastSwal({ title: 'Silindi!', text: 'Hammadde başarıyla silindi.', icon: 'success' });
                fetchData();
            } catch (error) {
                console.error('Error deleting ingredient', error);
                showSwal({ title: 'Hata', text: 'Hammadde silinirken bir sorun oluştu.', icon: 'error' });
            }
        }
    };
            ...
            <div className="w-full px-[50px] py-8 relative z-10">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div className="flex items-center">
                        ...
                        <div>
                            <h3 className="mb-0 text-3xl font-extralight text-emerald-600 dark:text-emerald-400 leading-none uppercase tracking-[0.25em]" id="title">HAMMADDE YÖNETİMİ</h3>
                            <h5 className="text-muted mb-0 text-lg font-medium text-slate-400 dark:text-slate-500 mt-0.5">Reçetelerde kullanılan hammadde ve sarf malzemeleri yönetin.</h5>
                        </div>
                    </div>
                    <div className="flex gap-3">
                        <button onClick={() => openModal()} className="px-6 py-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-black text-xs uppercase tracking-widest rounded-2xl shadow-sm hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition-all flex items-center gap-2 hover:scale-105 active:scale-95">
                            <i className="fat fa-plus-circle text-lg"></i> Yeni Hammadde
                        </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl p-6 rounded-[32px] border border-white dark:border-slate-700 flex items-center justify-between transition-all hover:border-emerald-300 dark:hover:border-emerald-500/40 hover:shadow-[0_8px_30px_-5px_rgba(16,185,129,0.3)] hover:scale-[1.02] cursor-pointer">
                        <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Toplam Hammadde</p>
                    ...
                    <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl p-6 rounded-[32px] border border-white dark:border-slate-700 flex items-center justify-between transition-all hover:border-teal-300 dark:hover:border-teal-500/40 hover:shadow-[0_8px_30px_-5px_rgba(20,184,166,0.3)] hover:scale-[1.02] cursor-pointer">
                        <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Aktif Hammaddeler</p>
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center p-20">
                        ...
                        <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Hammaddeler Yükleniyor...</p>
                    </div>
                ) : (
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead className="sticky top-0 z-10">
                                    <tr className="bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-700/50">
                                        ...
                                        <th className="px-8 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Hammadde Adı</th>
                                        <th className="px-8 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Barkod (SKU)</th>
                                        <th className="px-8 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Birim</th>
                                        <th className="px-8 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Maliyet Fiyatı</th>
                                ...
                                <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                                    {ingredients.map(ing => (
                                        <tr key={ing.id} className="hover:bg-emerald-500/5 dark:hover:bg-emerald-500/10 transition-all group">
                                            ...
                                            <td className="px-8 py-3">
                                                <span className="text-sm font-bold text-slate-700 dark:text-slate-300">{ing.unit || 'adet'}</span>
                                            </td>
                                        ...
                                        <tr>
                                            <td colSpan={6} className="p-20 text-center">
                                                <div className="flex flex-col items-center opacity-40">
                                                    ...
                                                    <p className="text-slate-500 font-bold uppercase tracking-widest text-sm">Kayıtlı Hammadde Bulunamadı</p>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
                        ...
                        <div className="p-8 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/20">
                            <div>
                                <h2 className="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-3 tracking-tighter uppercase">
                                    ...
                                    {formData.id === 0 ? 'YENİ HAMMADDE' : 'HAMMADDE DÜZENLE'}
                                </h2>
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Hammadde adı ve birim bilgilerini girin</p>
                            </div>
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div className="md:col-span-2">
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Hammadde Adı</label>
                                    <div className="relative">
                                        <i className="fat fa-leaf absolute left-4 top-3.5 text-emerald-500/50"></i>
                                        <input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-800 dark:text-white font-bold focus:ring-4 focus:ring-emerald-500/10 outline-none transition-shadow" placeholder="Hammadde adı" />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="text" required value={formData.sku} onChange={(e) => setFormData({ ...formData, sku: e.target.value })} className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-800 dark:text-white font-bold focus:ring-4 focus:ring-emerald-500/10 outline-none transition-shadow uppercase font-mono" placeholder="Barkod" />
                                    </div>
                                </div>
                                <div>
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Maliyet Fiyatı (₺)</label>
                                    <div className="relative">
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Kategori (Otomatik)</label>
                                    <div className="relative">
                                        <i className="fat fa-tag absolute left-4 top-3.5 text-slate-400"></i>
                                        <input type="text" readOnly value="Hammadde" className="w-full pl-12 pr-4 py-3.5 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-500 dark:text-slate-400 font-bold outline-none" />
                                    </div>
                                </div>
                            </div>
                            <div className="pt-6 flex flex-col gap-3">
                                <button type="submit" className="w-full py-4 bg-gradient-to-r from-emerald-600 to-teal-700 text-white rounded-[24px] font-black text-sm uppercase tracking-widest shadow-md shadow-emerald-500/20 hover:scale-[1.02] active:scale-95 transition-all">
                                    HAMMADDEYİ KAYDET
                                </button>
