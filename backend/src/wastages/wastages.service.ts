...
export class WastagesService {
    ...
    async getWastageSummary(startDate?: string, endDate?: string): Promise<{
        ...
        const productMap = new Map<number, { productName: string; totalQty: number; totalCost: number }>();
        ...
        for (const w of wastages) {
            ...
            if (!productMap.has(prodKey)) {
                productMap.set(prodKey, { productName: w.product?.name || '', totalQty: 0, totalCost: 0 });
            }
        ...
        return {
            ...
            byProduct: Array.from(productMap.entries()).map(([id, data]) => ({ productId: id, ...data })),
            byReason: Array.from(reasonMap.entries()).map(([reason, data]) => ({ reason, ...data })),
        };
    }
}
