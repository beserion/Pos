  ...
  async getDashboardData() {
    ...
    const weeklySalesRaw = await this.dataSource.query(`
      SELECT 
        ...
        SUM(amount) as total
      FROM account_transactions
      WHERE type = 'INCOME' AND createdAt >= DATEADD(day, -7, GETDATE())
    ...
    const netProfit = Number(totalIncome) - cogs - Number(totalExpense);

    return {
      totalIncome: Number(totalIncome), totalExpense: Number(totalExpense), cogs, netProfit,
