...
export class LicenseService implements OnApplicationBootstrap {
  ...
  // ─── 1 Saatlik Panel Önbelleği ──────────────────────────────────────────────
  // Panel sunucusuna gereğinden fazla istek atmamak için son doğrulama sonucunu
  // bellekte tutuyoruz. Polling (her 5 dk) geldiğinde bu önbellekten yanıt döner;
  // gerçek panel çağrısı sadece 1 saatte bir yapılır.
  private panelCache: {
    result: { valid: boolean; modules: string[] } | null;
    cachedAt: Date | null;
  } = { result: null, cachedAt: null };

  private readonly PANEL_CACHE_DURATION_MS = 60 * 60 * 1000; // 1 saat
  ...
  async saveLicense(licenseKey: string): Promise<any> {
    ...
    // ✅ YENİ EKLENEN KISIM: Eski önbelleği temizle ki bir sonraki check'te güncel lisans durumu panele sorulsun
    this.panelCache = { result: null, cachedAt: null };
  ...
  async checkLicenseStatus(): Promise<{ isValid: boolean; modules: string[]; daysOffline: number; details?: DecryptedLicense, reason?: string }> {
    ...
    // Online Verify Denemesi (1 Saatlik Önbellek ile)
    const panelUrl = process.env.PANEL_URL;
    const apiKey = process.env.LICENSE_API_KEY;
    ...
    if (panelUrl && apiKey) {
      const now = new Date();
      const cacheAge = this.panelCache.cachedAt
        ? now.getTime() - this.panelCache.cachedAt.getTime()
        : Infinity;
      const isCacheValid = cacheAge < this.PANEL_CACHE_DURATION_MS && this.panelCache.result !== null;

      if (isCacheValid) {
        // ─── Önbellekten yanıt dön (Panel'e istek atma) ───────────────────────
        this.logger.debug(`Panel önbelleği kullanılıyor (${Math.round(cacheAge / 60000)} dk önce güncellendi)`);
        const cached = this.panelCache.result!;
        if (!cached.valid) {
          return { isValid: false, modules: [], daysOffline: 0, reason: 'Lisans sunucu tarafından iptal edilmiş' };
        }
        onlineValid = true;
        onlineModules = cached.modules;
      } else {
        // ─── Önbellek süresi dolmuş veya yok → Panel'e gerçek istek at ────────
        try {
          this.logger.log('Panel doğrulaması yapılıyor...');
          ...
          const panelResult = { valid: !!response.data.valid, modules: response.data.modules || [] };
          // Önbelleği güncelle
          this.panelCache = { result: panelResult, cachedAt: new Date() };
          ...
          onlineValid = true;
          onlineModules = panelResult.modules;
          // ✅ Başarılı online check → son kontrol tarihini kaydet
          currentLicense.lastOnlineCheck = new Date();
          await this.licenseRepo.save(currentLicense);
        } catch (err) {
          // Panel'e ulaşılamadı → önbelleği temizle, offline moda geç
          this.panelCache = { result: null, cachedAt: null };
