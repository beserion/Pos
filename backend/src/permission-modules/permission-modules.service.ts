...
const DEFAULT_MODULES = [
  ...
  { key: 'POS_TABLE',   label: 'POS - Sipariş Ekranı',icon: 'fa-desktop',            color: 'text-teal-500',    accentBg: 'bg-teal-500/10',    actions: 'VIEW,ADD,EDIT,DELETE,PRINT,APPROVE', sortOrder: 9 },
  { key: 'POS_FAST',    label: 'POS - Hızlı Satış',  icon: 'fa-bolt',               color: 'text-orange-500',  accentBg: 'bg-orange-500/10',  actions: 'VIEW,ADD,EDIT,DELETE,PRINT,APPROVE', sortOrder: 9 },
  { key: 'SALES',       label: 'Satış Yönetimi',     icon: 'fa-cash-register',      color: 'text-emerald-500', accentBg: 'bg-emerald-500/10', actions: 'VIEW,ADD,EDIT,DELETE,PRINT,APPROVE', sortOrder: 10 },
  ...
  private async seedDefaults() {
    for (const mod of DEFAULT_MODULES) {
      ...
      if (!existing) {
        ...
        let changed = false;
        if (existing.label !== mod.label) { existing.label = mod.label; changed = true; }
        if (existing.icon !== mod.icon) { existing.icon = mod.icon; changed = true; }
        if (existing.color !== mod.color) { existing.color = mod.color; changed = true; }
        if (existing.accentBg !== mod.accentBg) { existing.accentBg = mod.accentBg; changed = true; }
        if (existing.actions !== mod.actions) { existing.actions = mod.actions; changed = true; }
        if (existing.sortOrder !== mod.sortOrder) { existing.sortOrder = mod.sortOrder; changed = true; }
        if (changed) {
            await this.repo.save(existing);
            console.log(`[PermissionModules] Updated module: ${mod.key}`);
        }
      }
    }
