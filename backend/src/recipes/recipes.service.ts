...
export class RecipesService {
    ...
    async calculateCost(productId: number): Promise<{ productId: number; productName: string; totalCost: number; items: any[] }> {
        ...
        const items = recipes.map(recipe => ({
            ...
            lineCost: Number(recipe.quantity) * Number(recipe.ingredient.costPrice || 0),
        }));
    ...
    async getRecipeSummary(productId: number): Promise<{
        ...
        const profitMarginPercent = salePrice > 0 ? (profitMargin / salePrice) * 100 : 0;
