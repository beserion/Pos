...
export class RecipesService {
  ...
  async getFullReport(): Promise<any[]> {
    ...
    // Sort by product name manually if needed, or use query builder
    ...
    return sortedHeaders.map((header) => {
      ...
      const items = (header.lines || []).map((line) => ({
        stockCardName: line.stockCard?.name || 'Bilinmiyor',
        quantity: Number(line.quantity),
        unit: line.unit,
        unitCost: Number(line.stockCard?.costPerBaseUnit || 0),
        lineCost: Number(line.quantity) * Number(line.stockCard?.costPerBaseUnit || 0),
      ...
      const costRatio = netSalePrice > 0 ? (totalCost / netSalePrice) * 100 : 0;

      return {
        productId: product?.id,
        productName: product?.name || header.name || 'İsimsiz Ürün',
